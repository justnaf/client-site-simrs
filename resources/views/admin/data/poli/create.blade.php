<x-admin-layout>
    ...
    <h2 class="text-3xl font-bold text-sky-800 mb-6"><a href="{{ route('admin.polis.index') }}"> Data Poli</a> > Tambah Harga</h2>

    @include('admin.includes.alert')

    {{-- Tampilkan error jika API gagal diakses --}}
    @if (isset($error))
    ...
    <div class="bg-white p-6 rounded-lg shadow-md border">
        <form action="{{ route('admin.polis.store') }}" method="POST" class="space-y-4">
            @csrf
            <div class="flex items-center">
                <label for="id_poli" class="w-48 font-medium text-gray-700">Pilih Poli</label>
                <span class="mr-4">:</span>
                <select id="id_poli" name="id_poli" required class="flex-1 border-gray-300 rounded-md shadow-sm focus:ring-sky-500 focus:border-sky-500 p-2">
                    <option value="">-- Pilih Poli yang Akan Diberi Harga --</option>
